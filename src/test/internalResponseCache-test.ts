...
import {Renderer, RendererConfig, ViewportDimensions} from '../renderer';

const app = new Koa();
app.use(koaStatic(path.resolve(__dirname, '../../test-resources')));

const defaultViewPortDimensions: ViewportDimensions = {
    width: 1000,
    height: 1000
};
...
test('rendering is not affected without cached config is not changed', async (t) => {
    const renderer = new Renderer(defaultRendererConfig);
    const res = await renderer.serialize(`${testBase}basic-script.html`, false, defaultViewPortDimensions);
    t.is(res.status, 200);
...
test('rendered content is same with cached config and without cached config is same', async (t) => {
    const rendererWithoutCache = new Renderer(defaultRendererConfig);
    const rendererWithCache = new Renderer(Object.assign({}, defaultRendererConfig, {
        internalRequestCacheConfig: {
    ...
    const resWithoutCache = await rendererWithoutCache.serialize(`${testBase}basic-script.html`, false, defaultViewPortDimensions);
    const resWithCache = await rendererWithCache.serialize(`${testBase}basic-script.html`, false, defaultViewPortDimensions);
...
test('url matching with cachUrlRegex would be cached', async (t) => {
    ...
    await rendererWithCache.serialize(`${testBase}script-after-load.html`, false, defaultViewPortDimensions);
    // @ts-ignore
...
test('first set cached response is removed if not used when maxEntries is breached', async (t) => {
    ...
    await rendererWithCache.serialize(`${testBase}basic-script.html`, false, defaultViewPortDimensions);
    // @ts-ignore
...
test('second set cached response is removed if first set response is used after second is set (verify lru logic)', async (t) => {
    ...
    await rendererWithCache.serialize(`${testBase}basic-script.html`, false, defaultViewPortDimensions);
    await rendererWithCache.serialize(`${testBase}basic-script.html?call=2`, false, defaultViewPortDimensions);


    await rendererWithCache.serialize(`${testBase}basic-script.html`, false, defaultViewPortDimensions);

    await rendererWithCache.serialize(`${testBase}basic-script.html?call=3`, false, defaultViewPortDimensions);
