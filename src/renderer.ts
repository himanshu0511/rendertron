    ...
    async serialize(requestUrl: string, isMobile: boolean):
        ...
        return await this.browserPool.acquire(async (browser: Browser) => {
            ...
            await page.evaluate(() => {
                ...
                document.querySelectorAll('[class*="Col-"]').forEach((element) => {
                    element.setAttribute('style', 'flex: auto;');
                });
                document.querySelectorAll('[class*="Row-"]').forEach((element) => {
                    element.setAttribute('style', 'flex: auto;');
                });
                document.querySelectorAll('[class^="ImageGallery__ImageGalleryCarousal-"]').forEach((element) => {
                    element.setAttribute('style', 'height: auto;');
                });
                document.querySelectorAll('[class^="ProductItem__SaleColWrapper-"]').forEach((element) => {
                    element.remove();
                });
            });
            // Serialize page.
